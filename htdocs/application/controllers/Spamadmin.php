...
class Spamadmin extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($pastes_to_delete) {
            foreach (explode(' ', $pastes_to_delete) as $pid) {
                // Fetch the paste to check if it has a file associated
                $this->db->select('file');
    ...
    public function del($dpid)
    {
        $this->load->model('pastes');
        $pastes_to_delete = $dpid;
        ...
        if ($pastes_to_delete) {
            foreach (explode(' ', $pastes_to_delete) as $pid) {
                ...
                // Delete corresponding entries from the 'trending' table
    ...
    public function spam_detail()
    {
        ...
        if ($this->input->post('confirm_remove') && $ip_address != '') {
            // Retrieve all pastes by the IP address before deletion
            $this->db->select('pid, file');
            $this->db->where('ip_address', $ip_address);
            $query = $this->db->get('pastes');

            // Loop through all pastes and delete associated files
            foreach ($query->result() as $paste) {
                if (!empty($paste->file)) {
                    $file_path = $this->config->item('upload_path') . $paste->file;
            ...
            // Now delete the pastes from the database
            $this->db->where('ip_address', $ip_address);
            $this->db->delete('pastes');
            ...
            if ($data['pastes'] != '') {
                foreach ($data['pastes'] as $trends) {
                    // Also remove them from trending
